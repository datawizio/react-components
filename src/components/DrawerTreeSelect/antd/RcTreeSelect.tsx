...
import generateSelector, { SelectProps, RefSelectProps } from "./RcSelect";
...
import OptionList from "rc-tree-select/es/OptionList";
import TreeNode from "rc-tree-select/es/TreeNode";
import {
  Key,
  DefaultValueType,
  DataNode,
...
import useKeyValueMap from "rc-tree-select/es/hooks/useKeyValueMap";
import useKeyValueMapping from "rc-tree-select/es/hooks/useKeyValueMapping";
import {
  CheckedStrategy,
  formatStrategyKeys,
  SHOW_ALL,
  SHOW_PARENT,
  SHOW_CHILD
} from "rc-tree-select/es/utils/strategyUtil";
import { fillAdditionalInfo } from "rc-tree-select/es/utils/legacyUtil";
import useSelectValues from "rc-tree-select/es/hooks/useSelectValues";
...
const RefTreeSelect = React.forwardRef<RefSelectProps, TreeSelectProps>(
  (props, ref) => {
    const {
      multiple,
      treeCheckable,
    ...
    const mergedCheckable: React.ReactNode | boolean =
    ...
    const flattedOptions = useMemo(() => flattenOptions(mergedTreeData), [
      mergedTreeData
    ]);
    ...
    // ========================= Value =========================
    const [value, setValue] = useMergedState<DefaultValueType>(
      props.defaultValue,
      {
        value: props.value
      }
    ...
    const [rawValues, rawHalfCheckedKeys]: [
      RawValueType[],
      RawValueType[]
    ] = useMemo(() => {
      ...
      if (treeConduction) {
        const { missingRawValues, existRawValues } = splitRawValues(
          newRawValues
        );
    ...
    const onInternalSelect = (
      ...
      if (!mergedMultiple) {
        ...
        triggerChange(
          [selectValue],
          { selected: true, triggerValue: selectValue },
          source
        );
      } else {
    ...
    const onInternalDeselect = (
      ...
      triggerChange(
        newRawValues,
        { selected: false, triggerValue: selectValue },
        source
      );
