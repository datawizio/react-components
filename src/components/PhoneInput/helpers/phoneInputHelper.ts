  ...
  if (new_country) {
    ...
    if (value[0] === "+") {
      ...
      if (preferNationalFormat) {
        ...
        if (
          value.indexOf("+" + getCountryCallingCode(new_country, metadata)) ===
          0
        ) {
      ...
      if (previous_country) {
        if (
          getCountryCallingCode(new_country, metadata) ===
          getCountryCallingCode(previous_country, metadata)
        ) {
          return value;
        } else {
          return `+${getCountryCallingCode(new_country, metadata)}`;
...
export function trimNumber(number, country, metadata) {
  ...
  if (nationalSignificantNumberPart) {
    const overflowDigitsCount =
      nationalSignificantNumberPart.length -
      getMaxNumberLength(country, metadata);
